...
public class TestUtil {
  ...
  protected static SherlockRequest createRequest(String input, String action,
      Map<String, String> params, SherlockRequest.Flag flag) {
    SherlockRequestBuilder builder =
        new SherlockRequestBuilder(input, TestUtil.CASE_SESSIONID, action).parameters(params)
  ...
  protected static void mockAssets(AssetsManager assetsManager, MessagesManager messagesManager)
      throws Exception {
    Mockito.when(messagesManager.message((String) any())).then(new Answer<String>() {
      @Override
    ...
    Mockito.when(messagesManager.message((String) any(), any())).then(new Answer<String>() {
      @Override
      public String answer(InvocationOnMock invocation) throws Throwable {
        return invocation.getArguments()[0].toString() + " message";
      }
  ...
  public static Case newCase(String id, boolean enabled) {
    return new Case(id, "data" + id, "name" + id, ImmutableList.of(), "category" + id, enabled,
        null);
  }

  public static Clue newClue(String id) {
    try {
      return new Clue(TestUtil.CASE_DATA_ID, id, id + " name", id + " desc", id + " keywords",
  ...
  public static Story story(String name) {
    return new Story(TestUtil.CASE_DATA_ID, name + "id", name, Story.LOCATION, name + " text", null,
        null, ImmutableList.of(), "latlong", new Date());
  }
}
